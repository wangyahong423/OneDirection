...
export default class Community extends Component {
                        ...
                        for (var i = 0; i < res.data.length; i++) {
                            for (var j = 0; j < r.data.length; j++) {
                                if (res.data[i].name == r.data[j].name) {
                                    ...
                                    if(r.data[j].card == 'null'){
                                        res.data[i].card = null;
                                    }
                                    else{
                                        res.data[i].card = "http://139.155.44.190:3005/card/" + r.data[j].card;
                                    }
                                }
                            }
                        }
    ...
    handleSend = (e) => {
        let url = `http://139.155.44.190:3005/learn/select?content=${e.target.value}`;
        axios(url)
            .then((res) => {
                if (res.data.false) {
                } else {
    ...
    changeSearch = (e) => {
        if (e.target.value == "") {
            let url0 = `http://139.155.44.190:3005/learn/list`;
            axios(url0)
                .then((res) => {
                    for (var i = 0; i < res.data.length; i++) {
                        res.data[i].pic = "http://139.155.44.190:3005" + res.data[i].pic;
            ...
            window.location.href = "http://localhost:3000/home#/community/";
        } else {
            this.setState({
