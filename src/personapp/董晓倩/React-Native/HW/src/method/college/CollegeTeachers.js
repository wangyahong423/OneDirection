        ...
        return (
            <ImageBackground style={{ width: '100%', height: height }} source={require('../../../assets/gonglve/beijing.jpg')}>
                <View style={{ width: width, height: 60 * s, flexDirection: "row", alignItems: "center", justifyContent: "center" }}>
                    ...
                    <Text style={{ fontSize: 22 * s }}>学院领导</Text>
                </View>
                        ...
                        <View style={styles.block}>
                            {
                                this.state.todo.map((item) => (
                                    <View style={{ paddingLeft: '10%' }}>
                                        <Text style={{ fontSize: 22*s }}>{item.job}</Text>
                                        <Text style={{ fontSize: 18*s }}>{item.name}</Text>
                                        <Text style={{ fontSize: 18 *s}}>{item.tel}</Text>
                                        <Text style={{ fontSize: 18*s, marginBottom: 10 }}>{item.email}</Text>
                                    </View>
                                )
                                )}
                        </View>
