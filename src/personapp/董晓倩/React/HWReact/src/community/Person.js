...
export default class Person extends Component {
                            ...
                            .then((res) => {
                                ...
                                axios(url4)
                                    .then((res) => {
                                        ...
                                        console.log('myelike:', this.state.myelike);
                                        //我赞过的经验分享帖子
                                        var perelikeList = [];
                                        for (var i = 0; i < res.data.length; i++) {
                                        ...
                                        axios(url7)
                                            .then((res) => {
                                                ...
                                                console.log('perCollect:', this.state.perCollect);
                                                var list3 = [];
    ...
    follow = () => {
        if (this.state.follow == false) {
            ...
            let url1 = `http://139.155.44.190:3005/follow/add?lname=${this.state.name}&nname=${this.state.username}`;
            let url3 = `http://139.155.44.190:3005/follow/changeP?lname=${this.state.name}&nname=${this.state.username}&newp=${true}`;
            axios(url1)
                .then((res) => {
                    if (res.data.ok) {
                        alert(res.data.msg);
            ...
            let url2 = `http://139.155.44.190:3005/follow/delete?nname=${this.state.username}`;
            axios(url2)
                .then((res) => {
                    if (res.data.ok) {
                        alert(res.data.msg);
                ...
                <div style={{ marginTop: '40vh' }}>
                    ...
                    >
                                    ...
                                    <div style={{ marginTop: '10vh' }}>
                                        {
                                            this.state.llikeData.map((item, idx) =>
                                                <div style={{ background: '#fff', color: 'black', marginBottom: '1vh', width: '100vw' }}>
                                                    ...
                                                    <Link to={`/perdetails/${this.state.data[0].name}/${item.id}`}>
                                                    ...
                                                    <div style={{ marginTop: 20 }}>
                                                        <Link to={`/percom/${this.state.data[0].name}/${item.id}`}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', backgroundColor: '#EFEFF4' }}>
                            <div style={{ position: "fixed", top: '2vh', width: "100vw", height: '6vh', backgroundColor: '#EFEFF4' }}>
                                <input placeholder='搜索' onChange={this.changeSearch} style={{ height: '5vh', borderRadius: '20px', border: 'none', marginTop: '0.5vh', textAlign: 'center', fontSize: '4vw', width: '85vw', float: 'left', borderRight: "none" }}></input>
                                <div onClick={this.mylSearch} className='iconfont icon-sousuo1' style={{ width: '15vw', float: 'left', height: '6vh', textAlign: 'center', color: '#999999', lineHeight: '6vh', fontSize: '4vw' }}></div>
                            </div>
                            <div style={{ width: '100vw', backgroundColor: '#EFEFF4' }}>
                                <div style={{ marginTop: '10vh' }}>
                                    {
