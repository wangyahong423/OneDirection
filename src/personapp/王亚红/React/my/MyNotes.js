...
export default class MyNotes extends Component {
    ...
    componentDidMount() {
        ...
        let url2 = `http://139.155.44.190:3005/notes/list`;
    ...
    changeSearch = (e) => {
        if (e.target.value == "") {
            window.location.href = "http://localhost:3000/mynotes";
        } else {
            this.setState({
        ...
        return (
            <div>
                ...
                <div style={{ width: "100vw", height: '7vh', backgroundColor: '#EFEFF4' }}>
                    <input placeholder='搜索笔记标题' onChange={this.changeSearch} style={{ height: '5vh', borderRadius: '20px', border: 'none', marginTop: '1vh', textAlign: 'center', fontSize: '4vw', width: '85vw', float: 'left', borderRight: "none" }}></input>
                    ...
                    <div>
                        {
                            this.state.data.map((item, idx) =>
                                <div style={{ background: '#fff', color: 'black', position: 'relative', height: '22vh', marginBottom: '1vh' }}>
                                    ...
                                    <Link to={`/mynotesdetails/${item.id}`}>
                                        <p style={{
                                            color: 'black', position: 'absolute', top: '9vh', left: '10vw', width: '80vw', fontSize: '19px', overflow: "hidden",
                                            textOverflow: "ellipsis",
                                            whiteSpace: "nowrap",
                                            fontWeight: 'bold',
                                        }}>{item.title}</p>
                                        <hr style={{
                                            color: '#ccc', position: 'absolute', top: '14.5vh', left: '10vw', width: '80vw', overflow: "hidden",
                                            textOverflow: "ellipsis",
                                            whiteSpace: "nowrap",
                                            fontWeight: 'bold'
                                        }}></hr>
                                        <p style={{
                                            color: 'black', position: 'absolute', top: '15vh', left: '10vw', width: '80vw', fontSize: '17px', overflow: "hidden",
                                            textOverflow: "ellipsis",
                                            textIndent: '2em',
                                            whiteSpace: "nowrap"
                                        }}>{item.content}</p>
